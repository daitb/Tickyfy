...
export const EventCard = React.memo(
  function EventCard({ event, onClick }: EventCardProps) {
    ...
    const { lowestPrice, highestPrice } = useMemo(() => {
      ...
      const ticketsToUse =
        availableTickets.length > 0 ? availableTickets : event.ticketTiers;

      const lowest =
        ticketsToUse.length > 0
    ...
    return (
      ...
      >
        <div className="aspect-[4/3] overflow-hidden bg-neutral-100 relative">
          ...
          />
          {/* Sold Out Badge */}
          {event.ticketTiers.every((t) => t.available === 0) && (
            <div className="absolute top-3 left-3 z-10">
              <Badge className="bg-red-600 text-white px-3 py-1 font-semibold shadow-lg">
                {t("common.soldOut")}
              </Badge>
            </div>
          )}
          {isAuthenticated && (
            <div className="absolute top-3 right-3 z-10">
              <WishlistButton eventId={eventId} size="md" />
            </div>
          )}
        </div>
        <div className="p-4">
          ...
          <h3 className="mb-1 line-clamp-2 min-h-[3em]">{event.title}</h3>
          ...
          <div className="mt-4 pt-4 border-t border-neutral-100 flex items-center justify-between">
            ...
            <span className="text-teal-600 font-semibold">
              {formatPriceRange(lowestPrice, highestPrice)}
            </span>
          </div>
        </div>
      </div>
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      JSON.stringify(prevProps.event.ticketTiers) ===
        JSON.stringify(nextProps.event.ticketTiers) &&
