...
export function OrganizerDashboard({ onNavigate }: OrganizerDashboardProps) {
  ...
  const totalSold = events.reduce((sum, event) => {
    const sold = Number(event.soldSeats) || 0;
    return sum + sold;
  }, 0);
  ...
  // Format monthly revenue for chart - ensure all values are valid numbers
  const salesData = (earnings?.monthlyRevenue || []).map((item: any) => ({
    ...item,
    ...
    return (
      <div className="min-h-screen bg-neutral-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => organizerId && loadDashboardData(organizerId)}>
            Retry
          </Button>
        </div>
      </div>
                  ...
                  <LineChart data={salesData}>
                    ...
                    <Tooltip
                      formatter={(value: any, name: string) => {
                        ...
                        if (name === "revenue") return formatPrice(numValue);
                      ...
                      >
                        ...
                        <div className="text-right">
                          <div className="text-neutral-900">
                            {formatPrice(Number(event.revenue) || 0)}
                          </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredEvents.map((event, index) => {
                        ...
                        const soldSeats = Number(event.soldSeats) || 0;
                        const salesRate =
                          totalSeats > 0
                            ? ((soldSeats / totalSeats) * 100).toFixed(1)
                            : "0";
                        const salesRateNum = parseFloat(salesRate) || 0;

                        return (
                          <TableRow key={event.eventId || `event-${index}`}>
                            <TableCell>
                              <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart
                      data={(earnings?.topEvents || []).map((event: any) => ({
                        ...event,
                ...
                <CardContent>
                  <div className="space-y-4">
                    {events.slice(0, 5).map((event, index) => {
                      ...
                      return (
                        <div key={event.eventId || `event-sales-${index}`}>
