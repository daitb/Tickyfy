...
export function EventListing({ onNavigate }: EventListingProps) {
  ...
  if (filters.city) {
    filteredEvents = filteredEvents.filter(e => 
      (e.city && e.city.toLowerCase().includes(filters.city!.toLowerCase())) ||
      (e.location && e.location.toLowerCase().includes(filters.city!.toLowerCase()))
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6 gap-4">
            ...
            <p className="text-sm text-neutral-600">
              {isLoading ? (
                ...
                <>
                  ...
                  {Object.keys(filters).length > 0 && ` ${t('events.withFilters', 'với bộ lọc')}`}
                </>
              )}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <span className="hidden sm:inline text-sm text-neutral-600">{t('events.sortBy', 'Sắp xếp')}:</span>
            <Select value={sortBy} onValueChange={(value) => setSortBy(value as SortOption)}>
              <SelectTrigger className="w-[140px] sm:w-[180px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="popularity">{t('events.sort.popularity', 'Phổ biến')}</SelectItem>
                <SelectItem value="date">{t('events.sort.date', 'Ngày')}</SelectItem>
                <SelectItem value="price-asc">{t('events.sort.priceLowToHigh', 'Giá thấp đến cao')}</SelectItem>
                <SelectItem value="price-desc">{t('events.sort.priceHighToLow', 'Giá cao đến thấp')}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
          ...
          >
            {filteredEvents.map((event) => (
              <EventCard
                ...
                event={event}
                onClick={() => onNavigate('event-detail', String(event.id || event.eventId))}
              />
            ))}
          </div>
        ) : (
