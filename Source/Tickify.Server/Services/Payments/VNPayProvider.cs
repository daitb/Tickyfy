...
public sealed class VNPayProvider : IPaymentProvider
{
    ...
    private async Task CreateTicketsForBookingAsync(int bookingId, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrEmpty(booking.SeatIdsJson))
        {
            try
            {
                seatIds = System.Text.Json.JsonSerializer.Deserialize<List<int>>(booking.SeatIdsJson);
                Console.WriteLine($"[VNPay] Found {seatIds?.Count ?? 0} seat IDs for booking {bookingId}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[VNPay] Error deserializing SeatIdsJson: {ex.Message}");
            }
        }
        ...
        {
            Console.WriteLine($"[VNPay] Warning: Seat count ({seatIds.Count}) doesn't match quantity ({quantity}) for booking {bookingId}");
        }
